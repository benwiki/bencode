...
def load_language_files():
    ...
    if not os.path.exists(LANG_DIR):
        print(f"Error: Directory '{LANG_DIR}' not found. Please create it and place your language JSON files inside.", file=sys.stderr)
        sys.exit(1)
...
def validate_and_collect_keys(lang_data):
    ...
    for lang_name, keys in keys_by_lang.items():
        ...
        if missing_keys:
            print(f"Error: Language '{lang_name}' is MISSING the following keys compared to '{canonical_name}': {', '.join(sorted(missing_keys))}", file=sys.stderr)
            all_valid = False
        
        if extra_keys:
            print(f"Error: Language '{lang_name}' has EXTRA keys not present in '{canonical_name}': {', '.join(sorted(extra_keys))}", file=sys.stderr)
            all_valid = False
            
    if not all_valid:
        print("\nKey mismatch detected. Please fix the JSON files before regenerating langtext.py.", file=sys.stderr)
        sys.exit(1)
        
    return sorted(list(canonical_keys))

def generate_langtext_py(lang_data, keys):
    ...
    for name in lang_data.keys():
        # Enum members are generated from file names (e.g., HUNGARIAN = 'languages/hungarian.json')
        filename = os.path.join(LANG_DIR, name.lower() + '.json').replace('\\', '/')
        content.append(f"    {name} = '{filename}'")

    content.extend("""    def text(self):
        return LangText(self)""".split("\n"))
    ...
    content.extend([
        ...
        f"            print(f\"ERROR: Language file {{lang_enum.value}} not found.\")",
        "        except json.JSONDecodeError:",
        f"            print(f\"ERROR: Language file {{lang_enum.value}} has invalid JSON.\")",
        "",
    ...
    for key in keys:
        ...
        content.append(f"        return self._text_data.get('{key}', 'MISSING TEXT KEY: {key}')")
        content.append(f"")
    ...
    try:
        with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
            f.write('\n'.join(content))
        print(f"Successfully generated {OUTPUT_FILE} with {len(keys)} keys from {len(lang_data)} languages.")
    except Exception as e:
