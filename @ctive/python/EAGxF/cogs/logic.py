...
class Date:
    ...
    def __str__(self):
        if self.hour is not None:
            return f"{self.day:02}.{self.month:02}.{self.year} {self.hour:02}:{self.minute:02}:{self.second:02}"
...
class Logic(commands.Cog):
    ...
    def __init__(self, client) -> None:
        ...
        self.structures = [
            Structure(
                id=2, message=
                    f"status: <status>\nWelcome to {self.app_name}, <name>! Click the buttons to use the app!",
                buttons=[
                    SimpleButton(
                        label="See Profile",
                        takes_to=3 ),
            ...
            Structure(
                ...
                buttons=[
                    ...
                    SimpleButton(
                        ...
                        takes_to=13 ),
                    SimpleButton(
                        label="Trust / Recommend",
                        takes_to=14 ),
                    SimpleButton(
                        label="Concern / Not Recommend",
                        takes_to=15 ),
            ]),
            Structure(
                id=10, message=
                    "Your current keywords are: <keywords>.\nWhat do you want to change your keywords to?"
            ...
            Structure(
                id=11, message=
                    ...
                    + "\n".join(f"{emoji} ({status.value})" for status, emoji in STATUS_EMOJI.items()),
                reactions=list(STATUS_EMOJI.values()),
                changes_property="status"),
            Structure(
                id=12, message=
                    "Your current answer to 'Where do I need help / what do I want to learn?' is: <need_help>."
                    "\nWhat do you want to change it to?",
                changes_property="need_help"),
            Structure(
                id=13, message=
                    "Your current answer to 'Where can I help / what is my expertise?' is: <can_help>."
                    "\nWhat do you want to change it to?",
                changes_property="can_help"),
            Structure(
                id=14, message=
                    "Your current answer to 'What do I trust / recommend?' is: <trust_recommend>."
                    "\nWhat do you want to change it to?",
                changes_property="trust_recommend"),
            Structure(
                id=15, message=
                    "Your current answer to 'What am I concerned about / what do I not recommend?' is: <concern_not_recommend>."
                    "\nWhat do you want to change it to?",
    ...
    async def on_message(self, msg: discord.Message):
        ...
        if user.change in ["name", "title", "location", "languages", "need_help", "can_help", "trust_recommend", "concern_not_recommend", "keywords"]:
            await self.delete_last_msg()
            ...
            changed = user.change
            if user.change in ["need_help", "can_help", "trust_recommend", "concern_not_recommend"]:
                changed = "answer"
            ...
            if user.change in ["keywords", "languages"]:
                ...
                self.msg_to_delete = await msg.channel.send(
                    f"✅ Your {changed} has been changed to \"{msg.content}\"!",
                    view=view)
            user.change = ""
            
    @commands.Cog.listener()
    async def on_reaction_add(self, reaction: discord.Reaction, dc_user: discord.User | discord.Member):
        ok_view = discord.ui.View()
        ok_btn: discord.ui.Button = discord.ui.Button(
            label="OK", style=discord.ButtonStyle.green)
        ok_view.add_item(ok_btn)
        ok_btn.callback = self.get_structure_callback( # type: ignore
            self.get_structure(4))  # type: ignore

        user = self.users[dc_user.id]
        if self.msg_to_delete is not None and reaction.message.id == self.msg_to_delete.id:
