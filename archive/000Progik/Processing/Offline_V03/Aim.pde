class Aim{
  ...
  Aim(String mode, Player pmaster){
    ...
    if (mode=="placing"){
      start = new Button("aimstart", pmaster.pos.x, pmaster.pos.y, pmaster.size.x, pmaster.size.y, 0,
                         0, color(255,0,0), 0, color(255,0,0), maxalpha/1.2, "circle");
      start.set_size(pmaster.size.x*1.5, pmaster.size.y*1.5, 250);
    }
    else
  ...
  void kill(){
    if (mode=="placing")
      start.set_size(pmaster.size.x, pmaster.size.y, 250);
    killing_it=true;
  }
