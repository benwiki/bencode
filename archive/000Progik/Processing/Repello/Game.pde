...
class Game{
  ...
  Handler handler;
  boolean players_placed=false, stepDone=false, empty = false;
  
  Player activePlayer =  emptyPlayer;
  Player currentPlayer = emptyPlayer;
  
  PVector aim_boardpos;
  /*ArrayList<ArrayList> playercopies = new ArrayList<ArrayList>();
  ArrayList<Disk> icons = new ArrayList<Disk>();*/
  ArrayList<Disk> gathered;
  Disk to_move;
  ...
  void check_cursor(){
    ...
    if (mouseX > bx + 3*board.cs && mouseX < bx +10*board.cs && 
      ...
      if (!activePlayer.empty && (!activePlayer.placeaim_directed() || (activePlayer.placeaim_directed() && activePlayer.moved_placeaim(aim_boardpos))))
  ...
  void push_out(){
    if (!pushout.pmaster.empty){
      ...
      to_move.pbutton.slide_backlog.add( new PVector(height+(width-height)/(players.size()+1)*(players.indexOf(to_move)+1.5), height-board.cs*1.5, -500));
      
      to_move.board_place.set(-1, -1);
      removeFromExe.add(to_move.place);
      removeFromExe.add(to_move.pbutton);
      buttonsToHide.add(to_move.pbutton);      
    }
    else {
      to_move = pushout.dmaster;
      to_move.movable.pos.set(board.boardc_to_c(to_move.pos).x,
                              board.boardc_to_c(to_move.pos).y);
      to_move.movable.slide(to_move.place.pos.x+cos(radians(pushout.arrowDir*45))*board.cs*2,
                            to_move.place.pos.y+sin(radians(pushout.arrowDir*45))*board.cs*2, 500, "whole");
      
      gathered = scoreboard.get_score(currentPlayer).get_disklist(to_move);
      
      /*for (int i=0; i<11; ++i)
        if (currentPlayer.pbutton.fcolor == handler.colors[i]){
          println(handler.colornames[i]);
          break;
        }*/
      int c = gathered.size()-1;
      for (Disk disk: gathered){
        disk.movable.slide( disk.movable.pos.x - board.cs/6 * pow(0.8, c), disk.movable.pos.y, 500, "whole");
        disk.movable.alpha = maxalpha * pow(0.8, c-- +1);
      }

      disks.remove(to_move);
      buttonsToHide.add(to_move.movable);
      removeFromExe.add(to_move.place);
      removeFromExe.add(to_move.movable);
      to_move.movable.set_size(board.cs, board.cs, 0);
      to_move.movable.slide_backlog.add(new PVector(height+(width-height)/(players.size()+1)*(players.indexOf(currentPlayer)+1.5), height+board.cs, 0));
      to_move.movable.slide_backlog.add(new PVector(height+(width-height)/(players.size()+1)*(players.indexOf(currentPlayer)+1.5), 
                                                    height/8*(to_move.movable.fcolor-1)+board.cs*3.5, -700));
    }
  }
  //-----------------------------------------------------------------------------------------------------------------------------------------------------------
