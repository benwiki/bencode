...
class Movements{
  ...
  ArrayList<Slide> all_slide, delete_slide;
  ...
  Slide newSlide (Button master){
    all.add( new Slide(master, this) );
    return (Slide)all.get(all.size()-1);
  }
  //--------------------------------------------------------------
  ...
  void runAll(){
    ...
    for (Slide s: delete_slide) all_slide.remove(s);
    for (Slide s: all_slide) s.run();
  }
  //----------------------------------------------------------------
...
class Slide {
  ...
  Slide my_slide = my_button.slide().setTime(1500)     // given in milliseconds
                                    .setMode("whole"); // possible modes: normal, whole (accelerate, then decelerate), accelerate, decelerate
  ...
  */
  ...
  Slide addDestination (PVector add_dest) {
    ...
    durations.add(add_dest.z); // ...the z component is the time amount.
    ...
    if (destinations.size()==1) distances.add(cur_dest.dist(mr.pos) );
    else distances.add( PVector.dist(destinations.get(destinations.size()-1), 
  ...
  void slideSetup() {
    ...
    for (int i=distances.size()-1; i>=0; --i)
      changeValue(distances, i, sumListUntil(distances, i));
    ...
    if (style == "united") {
      ...
      if (mode=="accelerate") {
        ...
        for (int i=0; i<destinations.size(); ++i)
          changeValue(durations, i, sqrt(2*distances.get(i)/acc.mag()));
      }
        ...
        vel.set( PVector.mult(acc, time) );
        for (int i=0; i<destinations.size(); ++i)
          changeValue(durations, i, (vel.mag() - sqrt(pow(vel.mag(), 2) - 2*distances.get(i)*acc.mag())) / acc.mag());
      }
      else if (mode=="whole") {
        ...
        for (int i=0; i<destinations.size(); ++i) {
          if (i == destinations.size()-1)
            changeValue(durations, i, time);
          else if (distances.get(i) <= fullDistance/2)
            changeValue(durations, i, sqrt(2*distances.get(i)/acc.mag()));
          else
            changeValue(durations, i, time/2 + (vel.mag() - sqrt(pow(vel.mag(), 2) - 2*(distances.get(i)-fullDistance/2)*acc.mag())) / acc.mag());
        }
      }
      else if (mode=="normal") {
        ...
        for (int i=0; i<destinations.size(); ++i)
          changeValue(durations, i, distances.get(i) / vel.mag());
      }
    }
    //..............................................................................................
    else if (style == "separated") {
