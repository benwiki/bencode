...
class Button{
  ...
  float fixw, fixh, fixd, scw, sch; // fix width, height, strokeWeight; sizechange width, height
  ...
  Button(String type, float x, float y, float w, float h, float d, color scolor, color fcolor, color ascolor, color afcolor, float alpha, String shape){
    ...
    if(shape=="disk" && gamemode == "desktop"){
      if (fcolor == 3) disk = requestImage("gold_disk.png");
  ...
  void change_size(){
    ...
    if (millis()-sizefix < sizetime){
      ...
      //d = map(cursizetime, 0, sizetime, fixd, fixd*scw/fixw);
    }
    else{
      sizechange = false;
      w = int(scw);
