...
def getwords(s, w, n, a):
    ...
    for i in range(len(w[n])):
        rajz.coords(wt[n][0], wt[n][1]-a[n]*45, 10+45*n)
        if n==len(w)-1:
            szo=''.join([w[k][a[k]] for k in range(len(w))])
            rajz.itemconfig(generated_word, text=szo)
            #if search(mygraph, szo) and not szo in osszes:
            if szo in wordsarr and szo not in osszes:
                tovabb=False
                wait_for_input()
                osszes.append(szo)
                rajz.create_text(x_bef+15, y_bef+len(words)+15, text=szo, font="Courier 10")
                if y_bef<h-30:
                    y_bef += 15
                else:
...
lightred='#FF7777'
gray='#555555'
lista=Listbox(bg=gray, fg='white', selectforeground=lightred, width=40, height=10)
...
try:
    ...
    wordsarr = dict.fromkeys(line.strip() for line in f.readlines())
    f.close()
...
w, h = ablak.winfo_screenwidth(), ablak.winfo_screenheight()*0.9
...
r=20
rajz.create_rectangle(w/2-r, 5, w/2+r, len(words)*45+5, outline='red', width=2)
word_text=[]
for i in range(len(words)):
    word_text.append([ rajz.create_text(w/2+len(words[i])/2*8,
                    ...
                    text = words[i],
                    font="Courier 10"),
               w/2+len(words[i])/2*35 ])
generated_word = rajz.create_text(w/2, len(words)*45+15, fill='red')
