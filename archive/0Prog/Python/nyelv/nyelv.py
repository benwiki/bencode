...
#-------------------------------
KIT = { 'mode':'apart', # mode: apart / both
    "magyar" : {},
    "német" : { 'mode':'and',
        "szófaj" : { 'mode':'or',
            "főnév" : { 'mode':'and',
...
class NyelvTanulas:
    ...
    def start(self):
        while self.running:
            whattodo = self.correct_input(self.menu, int, [i+1 for i in range(len(self.commands))])
    ...
    def settings(self):
        pass
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def correct_input(self, string, mytype, values, segment=False):
        ...
        while True:
            try:
                ...
                if question in values:
                    ...
                    for val in values:
                        if val.startswith(question): return val
            except Exception as e:
...
##        return ( len(given)==len(correct) and sum(1 for L in given  if L in correct) >= len(correct)-strength ) or ( 0 < abs(len(given)-len(correct)) <= strength and all(L in correct for L in given) )
        ...
        if glen==0 and clen>0: return False
        g=0; c=0; score=0
