...
def convert_to_map(string: str) -> Block:
    start: Block = Block(PROJECT_NAME, 0, None, [])
...
class MindMap:
    ...
    def draw_map(self, block: Block, x=None, y=None, r=None, rot=0.0):
        ...
        arcFill = self.prStyle.arcFill
        if self.prStyle.mode == "sector" and not 1 <= block.progress < 100:
            if block.progress == 100:
                circleFill = self.theme.pr
                arcFill = ""
            arcOutline = ""
        else:
            arcOutline = self.prStyle.arcOutline
    ...
    def scalezoom(self, pos):
        ...
        self.scalepos = pos
        self.do_zoom(factor, (self.draw_w/2, self.draw_h/2))

    def scrollzoom(self, event):
        factor = 1.001 ** event.delta
        self.do_zoom(factor, (event.x, event.y))

    def do_zoom(self, factor: float, pos: tuple[float, float]):
        self.resolution *= factor
        self.draw.scale("all",
                        pos[0] - self.draw_pos['x'],
                        pos[1] - self.draw_pos['y'],
                        factor, factor)
...
if __name__ == "__main__":
    mindmap_txt = extract_mindmap(PROJECT_NAME+".md")
