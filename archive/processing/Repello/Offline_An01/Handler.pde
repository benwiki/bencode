...
class Handler{
  ...
  void check_command(Button button){
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      //println("playerrrrrrrrr", gameStarted, game.currentPlayer==button.pmaster , !scoreboard.empty && scoreboard.graveyard.contains(button.pmaster));
    }
  ...
  void check_game(){                 // Function to run after the user added the players, and selected the colors //
    ...
    if (gameWillStart){ // placing the players... //
      if (!players.get(0).pbutton.sliding && !basicGameOperationDone){ // clearing and resetting "executable" //
        ...
        if(!game.players_placed()){
          ...
          game.set_scoreboard();
          game.indicators.add(new Indicator(players.get(0)));
        }
      }
    }
    /////////// loop of the game ////////////
    if (gameStarted){ 
      ...
      if (game.stepDone && !game.someone_is_sliding() && !game.has_tensions()){
        /// Step NOT Done yet! ///
        game.stepDone = false;
        game.currentPlayer = players.get(players.indexOf(game.currentPlayer)+1==players.size() ? 0 : players.indexOf(game.currentPlayer)+1);
