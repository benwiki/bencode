...
class Game{
  ...
  void set_scoreboard(){
    scoreboard = new Scoreboard("semmi. now it's legit");
  }
  //------------------------------------------------------------------------------------------------------------------------------------------------------
  
  void leave_disk(Player player){
    disks.add(new Disk(player.board_place.x, player.board_place.y, GRAY));
  }
  //-------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  void set_pushout_button(Button button, PVector pos, int dir){
    ...
    addToExe.add(pushout); // IMPORTANT!!!! if pushout isn't at the bottom of executable, the deletion of pushout happens before push_out(), and that causes some trouble.
  }
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  void push_out(){
    Disk disk_to_move;
     println("whatsgoingoooooon");
    if (!pushout.pmaster.empty){ //...... HA JÁTÉKOST TOLT LE ......//
      ...
      // Levonjuk a sarcot....
      scoreboard.get_score(to_move).addToDiskCounter(-1);
      if (scoreboard.get_score(to_move).outOfDisks()) to_move.dead = true;
      ...
      scoreboard.get_score(currentPlayer).get_disklist(GRAY).add(disk_to_move);
      
      // Meg további egy sarc a megszerzettjei közül
      scoreboard.activateRewards(to_move); // get_reward itt aktiválódik, ha a kilökött játékosnak van korongja.
    }
    
    else { //....... HA KORONGOT TOLT LE .........//
      println("whatsgoingon");
      disk_to_move = pushout.dmaster;
      // Adjuk hozzá a counterhez a megszerzett korongot
      scoreboard.get_score(currentPlayer).add_to_counter(disk_to_move.movable.fcolor, 1);
    ...
    organizeDisks(currentPlayer, disk_to_move.movable.fcolor);
    
    // Itt megpróbáltam lelőni a pushout_buttont, de csak feltámasztottam vele xd
    delete_pushout_button(); //<>//
  ...
  void organizeDisks (Player player, int col) {
    game.gathered = scoreboard.get_score(player).get_disklist(col);
    int c = game.gathered.size()-1;
    for (Disk d: game.gathered){
      d.movable.slide( height+(width-height)/(players.size()+1)*(players.indexOf(game.currentPlayer)+1.5) - board.cs/6 * pow(1.2, c), d.movable.pos.y, 500, "whole");
      d.movable.alpha = maxalpha * pow(0.8, c-- +1);
    }
  }
  //---------------------------------------------------------
  ...
  void executeReward (Button button) {
    ...
    disk = disks.get(disks.size()-1);
    disks.remove(disk);
    scoreboard.get_score(game.pushout.pmaster).add_to_counter(disk.movable.fcolor, -1);
    
    disk.movable.slide_backlog.add( new PVector(height+(width-height)/(players.size()+1)*(players.indexOf(game.currentPlayer)+1.5), height/8*(disk.movable.fcolor-1)+board.cs*3.5, 500) );
    scoreboard.get_score(game.currentPlayer).add_to_counter(disk.movable.fcolor, 1);
    
    game.organizeDisks(game.currentPlayer, disk.movable.fcolor);
    game.organizeDisks(game.pushout.pmaster, disk.movable.fcolor);
  ...
  Player getWinner(){
    ...
    for (Player player: players){
      if (scoreboard.get_score(player).sum() > max){
        max = scoreboard.get_score(player).sum();
        winner = player;
      }
    }
    if (winner.empty) println("No winner.");
    return winner;
  }
}
