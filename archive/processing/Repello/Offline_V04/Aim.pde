class Aim{
  ...
  PVector diff = new PVector(), boardgoal = new PVector(0, 0);
  Player pmaster;
  Disk dmaster;
  String mode;
  
  int breaks_done=0, waytime=800, placewaytime=300, wayfixtime, curtime;
  float wayWidth = board.cs/4;
  
  boolean empty = false, alive=true, killing_it=false, directed = false, goBack = false, destReached = false;
    ...
    if (alive){
      ...
      if (breakpoints.size()>1){
        ...
        if (breaks_done < breakpoints.size()-1 &&
            ...
            ++breaks_done;
            if (breaks_done == breakpoints.size()-1)
              middle.slide(end.pos.x, end.pos.y, waytime/breakpoints.size(), "whole");
            else
              middle.slide(board.boardc_to_c(breakpoints.get(breaks_done+1)).x,
                           board.boardc_to_c(breakpoints.get(breaks_done+1)).y, waytime/breakpoints.size(), "whole");
        }
      }
  ...
  void direct(PVector boardgoal){
    ...
    if(mode=="placing"){
      ...
      middle.slide(board.boardc_to_c(breakpoints.get(1)).x, 
                   board.boardc_to_c(breakpoints.get(1)).y, waytime/breakpoints.size(), "whole");
    else
      middle.slide(end.pos.x, end.pos.y, waytime, "whole");
  ...
  void kill(){
    if (mode == "placing" && pmaster.place.empty){
  ...
  void place_master(){
    if (game.place_taken(boardgoal)) return;
    if (!pmaster.empty){
      if (pmaster.placed) pmaster.move(this, boardgoal);
