...
class Brain{
  ...
  void randomize(){
    for(int i = 0; i < 5; ++i)
       for(int j = 0; j < 8; ++j) 
         intohidden[i][j] = random.nextFloat()*2-1;
    for(int i = 0; i < 8; ++i)
       for(int j = 0; j < 4; ++j)
         hiddentoout[i][j] = random.nextFloat()*2-1;
  }
  ...
  void mutate(float rate, int chance){
    for(int i = 0; i < 5; ++i)
      for(int j = 0; j < 8; ++j)
        if(int(random(chance + 1)) == chance) this.intohidden[i][j] = constrain(this.intohidden[i][j] + random.nextFloat()*2*rate-rate, -1, 1);
    for(int i = 0; i < 8; ++i)
      for(int j = 0; j < 4; ++j)
        if(int(random(chance + 1)) == chance) this.hiddentoout[i][j] = constrain(this.hiddentoout[i][j] + random.nextFloat()*2*rate-rate, -1, 1);
  }
