 ...
 /*
  ...
  void mutate(Brain[] parents){
    ...
    for(int k = 0; k < popsize; ++k){
      brains[k] = copy_brain(parents[k % parents.length]);
      //print(cars[k].score);
      if (k > 0) brains[k].mutate(.18, 5); //.25/pow(cars[k].score, 0.1)
    }
    println("");
    //countAverages();
  }
  
  Brain copy_brain (Brain b){
    Brain new_b = new Brain();
    for(int i = 0; i < 5; ++i)
      for(int j = 0; j < 8; ++j)
        new_b.intohidden[i][j] = b.intohidden[i][j];
    for(int i = 0; i < 8; ++i)
       for(int j = 0; j < 4; ++j)
         new_b.hiddentoout[i][j] = b.hiddentoout[i][j];
    return new_b;
  }
