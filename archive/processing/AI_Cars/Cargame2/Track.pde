class Track{
 ...
 PVector[] posin;
  ...
  Track(int num){
    readposout("/Tracks/trackout"+str(num)+".csv");
    readposin("/Tracks/trackin"+str(num)+".csv");
    show();
  }
  ...
  void readposout(String filename){
    ...
    for(int i = 0; i < posout.length; ++i){
      posout[i] = new PVector(float(split(lines[i], ';')[0]), 
                             float(split(lines[i], ';')[1]));
    }
  }
  ...
  void readposin(String filename){
    ...
    for(int i = 0; i < posin.length; ++i){
      posin[i] = new PVector(float(split(lines[i], ';')[0]), 
                             float(split(lines[i], ';')[1]));
    }
  }
  ...
  boolean lineline(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
    ...
    if (uA >= 0 && uA <= 1 && uB >= 0 && uB <= 1){
      ...
      /*stroke(204, 102, 0);
      ellipse(x1 + uA * (x2-x1), y1 + uA * (y2-y1), 10, 10);
      line(x3, y3, x4, y4);
      line(x1, y1, x2, y2);
      stroke(0);*/
