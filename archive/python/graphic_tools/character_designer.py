...
def redo():
        global drawing_object, cur_index
...
def motion(event):
        ...
        if not pressed and letsdraw:
                ...
                if hold_press==0 and n==2:
                        hold_press=time()
                lastcoord=[-1, -1]
                rajz.itemconfig(buttonToModify, fill=main_color)

def release(event):
        global letsdraw; letsdraw = 0
        x, y = event.x, event.y
        n, pressed=pressed_menu_button(x, y)
        if pressed:
                if n==1:
...
while 1:
        ablak.update()
        hold_buttons()
        rajz.itemconfig(text, text=str(valid_lines)+' '+str(cur_index))
